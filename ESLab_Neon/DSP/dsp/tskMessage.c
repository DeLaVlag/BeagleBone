...
Uint32 result[ARG_SIZE][ARG_SIZE];
...
Int TSKMESSAGE_execute(TSKMESSAGE_TransferInfo* info)
{
    ...
    for (i = 0; (((info->numTransfers == 0) || (i < info->numTransfers)) && (status == SYS_OK)); i++)
    {
        ...
        if (status == SYS_OK)
        {
            ...
            {
			/*=============== MESSAGE RECEIVED - SAVE LOCALLY=============*/
			/* Include your control flag or processing code here */
				for (x= 0;x < ARG_SIZE; x++)
				{
					for (y = 0; y < ARG_SIZE; y++)
					{
						mat1[x][y] = msg->mat1[y+x*ARG_SIZE];
						mat2[x][y] = msg->mat2[y+x*ARG_SIZE];
					}
				}
				for (x= 0;x < ARG_SIZE; x++)
				{
					for (y = 0; y < ARG_SIZE; y++)
					{
						...
						for(k=0;k<ARG_SIZE;k++)  
							result[x][y] = result[x][y]+mat1[x][k] * mat2[k][y];
					}
				}			
				for (x= 0;x < ARG_SIZE; x++)
				{
					for (y = 0; y < ARG_SIZE; y++)
				 		msg->mat2[y+x*ARG_SIZE]=result[x][y];
