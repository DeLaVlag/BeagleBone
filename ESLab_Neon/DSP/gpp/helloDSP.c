...
#define NUMMSGINPOOL3   4
    ...
    typedef struct ControlMsg
    {
        ...
        Uint16 command;
        Char8 arg1[ARG_SIZE];
		Uint32 mat1[ARG_SIZE*ARG_SIZE]; 
		Uint32 mat2[ARG_SIZE*ARG_SIZE];
    } ControlMsg;
...
#define SAMPLEMQT_CTRLMSG_SIZE  ZCPYMQT_CTRLMSG_SIZE
    ...
    NORMAL_API DSP_STATUS helloDSP_Execute(IN Uint32 numIterations, Uint8 processorId)
    {
        ...
        for (i = 1 ; ((numIterations == 0) || (i <= (numIterations + 1))) && (DSP_SUCCEEDED (status)); i++)
        {
            ...
            {
				/*======CREATE MATRIX 1========================*/
				for(k1 = 0; k1<ARG_SIZE; k1++)
					for(k2 = 0; k2<ARG_SIZE; k2++)
						msg->mat1[k2+k1*ARG_SIZE] = 1;
				/*============MATRIX 1 COMPLETE================*/
                /*======CREATE MATRIX 2========================*/
				for(k1 = 0; k1<ARG_SIZE; k1++)
					for(k2 = 0; k2<ARG_SIZE; k2++)
						msg->mat2[k2+k1*ARG_SIZE] = 2;
				/*============MATRIX 2 COMPLETE================*/
...
#endif
				//SYSTEM_0Print("Ready to receive..\n");
				/*=========RECEIVE MATRIX======================== */
				status = MSGQ_get(SampleGppMsgq, WAIT_FOREVER, (MsgqMsg *) &msg);
				stopTimer(&totalTime); //PHOTOFINISH
				//SYSTEM_0Print("GMT!!\n");
				if (DSP_FAILED(status))
				{
					SYSTEM_1Print("MSGQ_get () failed. Status = [0x%x]\n", status);
				...
				for(k1 = 0; k1<ARG_SIZE; k1++)
					for(k2 = 0; k2<ARG_SIZE; k2++)
					{
						SYSTEM_1Print("Message received[%d][",k1);
						SYSTEM_1Print("%d]:",k2);
						SYSTEM_1Print(" %d\n", msg->mat1[k2+k1*ARG_SIZE]);
					}
				for(k1 = 0; k1<ARG_SIZE; k1++)
					for(k2 = 0; k2<ARG_SIZE; k2++)
					{
