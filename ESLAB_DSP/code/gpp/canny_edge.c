...
int canny_edge_main(int arg1, char *arg2, unsigned char *image, int rowsPool, int colsPool, short int *smoothedim) /*----- Changed to canny_edge_main-> no conflict with dsp main main -----*/
{
    ...
    /*----------------- The input image is now an argument */
    unsigned char *edge;      /* The output edge image */
    int rows=rowsPool, cols=colsPool;           /* The dimensions of the image. */
    ...
    * Read in the image. This read function allocates memory for the image.
    This is done in main now. Turned on to see if it works and in it does
    ****************************************************************************/
    //if(VERBOSE) printf("Reading the image in canny%s.\n", infilename);
    ...
    ****************************************************************************/
    ...
    //MCPROF_START();
    canny(image, rows, cols, sigma, tlow, thigh, &edge, dirfilename, smoothedim);
           ...
           float tlow, float thigh, unsigned char **edge, char *fname, short int *smoothedim)
{
    ...
    short int //*smoothedim,     /* The image after gaussian smoothing.Is now a variable in pool notify      */
                   ...
                   short int *magnitude)
{
	...
	int i;
	int32_t temp[4];
	int32x4_t vec_sq1, vec_sq2, vec_sum;
	...
	for(r=0,pos=0; r<rows; r++)
    {	
		// use NEON to multiply four at a time
		for(c=0; c<(cols-4); c+=4){
			// load row vector with four values
			for(i=0; i<4; i++)
				temp[i] = (int32_t)delta_x[pos+i];
			vec_sq1 = vld1q_s32(temp);
			for(i=0; i<4; i++)
				temp[i] = (int32_t)delta_y[pos+i];
			vec_sq2 = vld1q_s32(temp);
			// multiply vectors (vec_sq1 = vec_sq1 * vec_sq1)
			vec_sq1 = vmulq_s32(vec_sq1, vec_sq1);
			// accumulate the two vectors (vec_sum = vec_sq1 + vec_sq2 * vec_sq2)
			vec_sum = vmlaq_s32(vec_sq1, vec_sq2, vec_sq2);
			// extract values
			vst1q_s32((int32_t*) temp, vec_sum);
			// do the final calculations
			for(i=0; i<4; i++){
				magnitude[pos] = (short)(0.5 + sqrt((float)temp[i]));
				pos++;
			}
		}
		// sweep up everything smaller then four without the NEON
		for(c=c; c<cols; c++, pos++){
			sq1 = (int)delta_x[pos] * (int)delta_x[pos];
            sq2 = (int)delta_y[pos] * (int)delta_y[pos];
            magnitude[pos] = (short)(0.5 + sqrt((float)sq1 + (float)sq2));
		}
...
* DATE: 2/15/96
*******************************************************************************/
void gaussian_smooth(unsigned char *image, short int *smoothedim, int rows, int cols, float *kernel, int start, int windowsize)
{
    int r, c, rr, cc,     /* Counter variables. */
        center;            /* Half of the windowsize. */
	int i, index;				
    float32_t *tempim,        /* Buffer for separable filter gaussian smoothing. */
          dot,            /* Dot product summing variable. */
          sum;            /* Sum of the kernel weights variable. */
	float32_t vImage[4];
	float32_t vdot[4], vsum[4];
	float32x4_t vec_dot, vec_sum, vec_image, vec_kernel;
    ...
    ****************************************************************************/
    ...
    //kernel = (float *) malloc((windowsize)* sizeof(float));
	
	/*float kernel2[]  = 
			{	0.00317, 0.00898, 0.02165, 0.04448, 0.07787,
				0.11617, 0.14768, 0.15998, 0.14768,	0.11617,
				0.07787, 0.04448, 0.02165, 0.00898, 0.00317
			 };
	*/		 
	/*kernel = kernel2;*/
	center = windowsize / 2;
    ...
    ****************************************************************************/
	...
	for(r=start; r<rows; r++){
        for(c=0; c<cols; c++){
			// initialize neon variable
			vec_dot = vdupq_n_f32(0.0);
			vec_sum = vdupq_n_f32(0.0);
            dot = 0.0;
            sum = 0.0;
			// process with neon in batches of 4
            for(cc=(-center); cc<=(center-4); cc+=4){
				if((c+cc) < 0){
					rr-=3; // increase by 1
				}
				else if((c+cc+4) >= rows){
					...
					// convert to float
					for(i=0;i<4;i++){
						vImage[i] = (float32_t) image[index+cc+i];
					}
					// load into neon
					vec_image = vld1q_f32(vImage);
					vec_kernel = vld1q_f32((float32_t *)&kernel[center+cc]);
					// calculate using neon
					vec_dot = vmlaq_f32(vec_dot, vec_image, vec_kernel);
					vec_sum = vaddq_f32(vec_sum, vec_kernel);
                }
            }
			// get vector values
			vst1q_f32(vdot, vec_dot);
			vst1q_f32(vsum, vec_sum);
			// add every entry into a scalar
			dot = vdot[0] + vdot[1] + vdot[2] + vdot[3];
			sum = vsum[0] + vsum[1] + vsum[2] + vsum[3];
			// sweep up batches smaller then 4 without the neon
			for(cc=cc; cc<=center; cc++){
                if(((c+cc) >= 0) && ((c+cc) < cols)){
                    dot += (float32_t)image[index+cc] * (float32_t)kernel[center+cc];
                    sum += (float32_t)kernel[center+cc];
                }
            }
            tempim[index] = dot/sum;
			index++;
        }
    }
    /****************************************************************************
    ...
    ****************************************************************************/
	...
	for(c=0; c<cols; c++){
        for(r=start; r<rows; r++){
            ...
            sum = 0.0;
			// process with neon in batches of 4
			for(rr=(-center); rr<=(center-4); rr+=4){
				if((r+rr) < 0){
					rr-=3; // increase by 1
				}
				else if((r+rr+4) >= rows){
					...
					// convert to float
					for(i=0;i<4;i++){
						vImage[i] = tempim[(r+rr+i)*cols+c];
					}
					// load into neon
					vec_image = vld1q_f32(vImage);
					vec_kernel = vld1q_f32((float32_t *)&kernel[center+rr]);
					// calculate using neon
					vec_dot = vmlaq_f32(vec_dot, vec_image, vec_kernel);
					vec_sum = vaddq_f32(vec_sum, vec_kernel);
                }
			...
			sum = vsum[0] + vsum[1] + vsum[2] + vsum[3];
			// sweep up batches smaller then 4 without the neon
            for(rr=rr; rr<=center; rr++){
                if(((r+rr) >= 0) && ((r+rr) < rows)){
                    dot += tempim[(r+rr)*cols+c] * (float32_t)kernel[center+rr];
                    sum += (float32_t)kernel[center+rr];
                }
            }
            smoothedim[r*cols+c] = (short int)(dot*BOOSTBLURFACTOR/sum + 0.5);
        }
